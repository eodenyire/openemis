...
# --------------------------------------------------------------------------- #
# Seed ERP & financial demo data
# --------------------------------------------------------------------------- #
info "Seeding ERP & financial demo data (payslips, LPOs, payments, discipline, assessments)…"
python3 scripts/seed_erp_data.py \
    --url "http://${ODOO_HOST}:${ODOO_PORT}" \
    --db "${ODOO_DB}" \
    --user admin \
    --password "${ODOO_ADMIN_PASSWD}" || warn "ERP data seeding returned non-zero exit code – check output above."
info "ERP data seeding complete."
...
info "Run 'bash scripts/e2e_regression_test.sh' to run all functional regression tests."
info "Run 'bash scripts/verify_system.sh' to query the database."
info "Run 'python3 scripts/capture_screenshots.py' to capture UI screenshots."
