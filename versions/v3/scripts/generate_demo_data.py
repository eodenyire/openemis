...
Generates Odoo XML demo-data files that populate the database with:
  - 100 classrooms       (openemis_classroom)
  - 100 staff / faculty  (openemis_core)
  - 100 parents          (openemis_parent)
  - 100 students         (openemis_core, 100 per course/class)
...
DEPARTMENTS = [
    "openemis_core.op_department_1",
    "openemis_core.op_department_2",
    "openemis_core.op_department_3",
    "openemis_core.op_department_4",
]

# courses with at least one batch defined
COURSES_WITH_BATCHES = [
    ("openemis_core.op_course_1", "openemis_core.op_batch_1"),
    ("openemis_core.op_course_2", "openemis_core.op_batch_2"),
    ("openemis_core.op_course_3", "openemis_core.op_batch_3"),
    ("openemis_core.op_course_5", "openemis_core.op_batch_4"),
    ("openemis_core.op_course_8", "openemis_core.op_batch_5"),
    ("openemis_core.op_course_1", "openemis_core.op_batch_6"),
    ("openemis_core.op_course_5", "openemis_core.op_batch_7"),
    ("openemis_core.op_course_7", "openemis_core.op_batch_8"),
    ("openemis_core.op_course_9", "openemis_core.op_batch_9"),
    ("openemis_core.op_course_10", "openemis_core.op_batch_10"),
]

FACULTY_SUBJECTS = [
    "openemis_core.op_subject_1",
    "openemis_core.op_subject_2",
    "openemis_core.op_subject_5",
    "openemis_core.op_subject_26",
    "openemis_core.op_subject_27",
]

PARENT_RELATIONSHIPS = [
    "openemis_parent.op_parent_relationship_1",  # Father
    "openemis_parent.op_parent_relationship_2",  # Mother
    "openemis_parent.op_parent_relationship_3",  # Guardian
]
...
def generate_classrooms():
    """Generate 100 classroom records for openemis_classroom."""
    ...
    for i in range(1, 101):
        ...
        <field name="course_id" ref="openemis_core.op_course_{course_num}"/>
    </record>''')
    lines.append('</odoo>\n')
    path = os.path.join(BASE_DIR, "openemis_classroom", "demo",
...
def generate_faculty():
    """Generate 100 faculty partner + faculty records for openemis_core."""
    ...
    _write(os.path.join(BASE_DIR, "openemis_core", "demo",
...
def generate_parents():
    """Generate 100 parent partner + user + parent records for openemis_parent."""
    partner_lines = ['<?xml version="1.0" encoding="utf-8"?>', '<odoo noupdate="1">']
    ...
    for i in range(1, 101):
        ...
        <field name="name" ref="openemis_parent.op_res_partner_bulk_p_{i:03d}"/>
        <field name="student_ids" eval="[(6,0,[ref('openemis_core.op_student_bulk_{stud_a:03d}'),ref('openemis_core.op_student_bulk_{stud_b:03d}')])]"/>
        <field name="relationship_id" ref="{rel}"/>
        <field name="user_id" ref="openemis_parent.op_user_parent_bulk_{i:03d}"/>
    </record>''')
    ...
    _write(os.path.join(BASE_DIR, "openemis_parent", "demo",
...
def generate_students():
    """Generate 100 student partner + student records for openemis_core.
...
def patch_manifests():
    ...
    # openemis_core
    core_manifest = os.path.join(BASE_DIR, "openemis_core", "__manifest__.py")
    for entry, anchor in [
    ...
    # openemis_classroom
    classroom_manifest = os.path.join(BASE_DIR, "openemis_classroom",
                                      "__manifest__.py")
    ...
    # openemis_parent
    parent_manifest = os.path.join(BASE_DIR, "openemis_parent", "__manifest__.py")
    for entry, anchor in [
