...
class TestCore(TestCoreCommon):
    ...
    def test_case_3_faculty(self):
        ...
        new_faculty = self.op_faculty.create({
            'partner_id': self.env.ref('openemis_core.op_res_partner_30').id,
    ...
    def test_case_5_res_company(self):
        ...
        record = self.env.ref('openemis_core.op_faculty_1'),
        users.create_user(record)

    def test_case_5_school_profile(self):
        ...
        company.write({
            ...
            'registration_number': 'SCH/2000/001',
        })
        self.assertEqual(company.principal_name, 'Dr. Jane Doe')
        self.assertEqual(company.school_motto, 'Excellence in Education')
        self.assertEqual(company.school_type, 'secondary')
        self.assertEqual(company.registration_number, 'SCH/2000/001')
    ...
    def test_case_6_student(self):
        ...
        vals = {
            ...
            'gender': 'm',
            'partner_id': self.env.ref('openemis_core.op_res_partner_14').id
    ...
    def test_case_7_subject_registartion(self):
        vals = {
            'student_id': self.env.ref('openemis_core.op_student_1').id,
            'course_id': self.env.ref('openemis_core.op_course_2').id,
            'batch_id': self.env.ref('openemis_core.op_batch_1').id,
        }
    ...
    def test_case_9_faculty_user_wizard(self):
        ...
        faculty_id = self.env.ref('openemis_core.op_faculty_1')
    ...
    def test_case_10_studnet_wizard(self):
        student_id = self.env.ref('openemis_core.op_student_1')
